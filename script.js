...
const mater = new THREE.MeshStandardMaterial({ color: 0xffff00 });
const spher = new THREE.Mesh(geom, mater);
scene.add(spher);
spher.position.x = -100

const ge = new THREE.SphereBufferGeometry(5, 32, 32);
const mate = new THREE.MeshStandardMaterial({ color: 0xfff0000 });
const sphe = new THREE.Mesh(ge, mate);
scene.add(sphe);
sphe.position.x = 100
...
const loop = () => {
    ...
    const distance_1 = spaceshipGroup.position.distanceTo(spher.position);
    const distance_2 = spaceshipGroup.position.distanceTo(sphe.position);
    if (distance_2 < 5) {
        alert('YOUUU WIIIN')
    }
    if (distance_1 < 5) {
    ...
    if (leap && leap.hands && leap.hands.length > 0) {
        spaceshipGroup.position.x += leap.hands[0].palmPosition[0] * 0.01;

    }

    if (leap && leap.hands[0] && leap.hands[0].pinchStrength > 0) {
        spaceshipGroup.position.z += -5
    }

    if (leap && leap.hands[0] && leap.hands[0].grabStrength > 0) {
        spaceshipGroup.position.z += 5
    }
